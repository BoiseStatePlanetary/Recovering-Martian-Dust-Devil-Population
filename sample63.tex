...
\end{equation}
where $V_{\rm act}$ is the tangential wind speed at the vortex diameter. Similar to the pressure signal, a non-zero $b$ means the maximum wind speed encountered $V_{\rm obs}$ is less than the actual maximum at the vortex diameter. We model the observed vortex wind profile is given by 
\begin{equation}
...
\begin{equation}
    ...
    \end{array}\right,\label{eqn:total_wind_speed}
\end{equation}
...
As discussed below, the wind time-series have limited time resolution and exhibit correlated noise, both of which make robust model fits difficult. To bolster our results, we also fit a second, more simple wind profile:
\begin{equation}
    V(r) = \left\{
        \begin{array}{ll}
...
\end{equation}
(For both this simpler and the full wind profile, $V_{\rm obs}$ may be negative, reflecting cyclonic or anti-cyclonic vortices.) We compare the fit parameters between the two models for each encounter to corroborate our results.

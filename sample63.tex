...
\section{Pressure Time-Series and Vortex Detection Scheme}
...
\end{equation}
where $t$ is the time of each datum, $\Delta P_{\rm act}$ is the pressure excursion (a positive number) at the center of vortex signal (the ``actual'' value, as contrasted with the ``observed'' value -- see below), $t_0$ the central time, and $\Gamma_{\rm act}$ the actual full-width/half-max (FWHM) for the profile. Assume, also, a simple linear trajectory for the vortex with a ground velocity $U$ (very nearly equal to the ambient wind speed -- \citealp{2012Icar..221..632B}), and take the vortex's physical diameter $D_{\rm act} = U\ \Gamma_{\rm act}$. The radial distance between the barometer and the vortex center $r(t) = \sqrt{U \left( t - t_0 \right)^2 + b^2}$, where $b$ is the closest approach distance during the whole encounter. 
...
How do these distortion effects influence the recovery of an underlying vortex population? As an example, assume all vortices have the same $\Delta P_{\rm act}$- and $D_{\rm act}$-values -- typical values for Mars are $\Delta P_{\rm act} = 1\,{\rm Pa}$ and $D_{\rm act} = 10\,{\rm m}$. With $U = 1\,{\rm m\ s^{-1}}$, $\Gamma_{\rm act} = 10\, {\rm s}$. (In reality, these parameters exhibit a wide but highly skewed distribution, rendering ``typical'' values of dubious merit -- \citealp{2016SSRv..203..277L}.) Some detection schemes sift the pressure time-series for individual outlier points with minimum threshold pressure excursions $\Delta P_{\rm obs} \geq \Delta P_{\rm min}$ \citep{2010JGRE..115.0E16E, 2020arXiv200501134S}, usually set by the noise in the time-series $\sigma_{\rm P}$.

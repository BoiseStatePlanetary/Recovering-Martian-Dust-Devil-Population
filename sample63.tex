...
Figure \ref{fig:U_vs_t0-sol_hist} shows the wind speeds associated with the observed vortices; the maximum, median, and minimum values are $19.2\pm1.6$, $9.30\pm0.73$, and $0.85\pm0.01 \,{\rm m\ s^{-1}}$, respectively. Figure \ref{fig:Dobs_vs_Gammaobs-DeltaPobs_hist} shows the distribution of $D_{\rm obs}$-values against $\Gamma_{\rm obs}$ and $\Delta P_{\rm obs}$, as well as a cumulative histogram. The maximum, minimum, and median values are $1260 \pm 500$, $97.2 \pm 18.1$, and $5.43 \pm 7.23 \,{\rm m}$, respectively. Not surprisingly, the $D_{\rm obs}$-values correlate positively with $\Gamma_{\rm obs}$ with scatter resulting from the relatively narrow range of wind speeds. 
On the other hand, $D_{\rm obs}$ seems negatively correlated with $P_{\rm obs}$, and a power-law fit using the Leverberg-Marquardt algorithm \citep{Press2007} gives $D_{\rm obs} \propto \Delta P_{\rm obs}^{-0.6\pm0.04}$, where uncertainties come from the covariance matrix. As explained in \citet{2018Icar..299..166J}, for a vortex encounter with any miss distance $b$, we expect $\Delta P_{\rm obs}$ and $D_{\rm obs}$ are related to the vortex's central pressure excursion $\Delta P_{\rm act}$ and diameter $D_{\rm act}$ as $\Delta P_{\rm obs} D_{\rm obs}^2 = \Delta P_{\rm act} D_{\rm act}^2$. Figure \ref{fig:Dobs_vs_Gammaobs-DeltaPobs_hist}(b) could be interpreted, then, as encounters with vortices at a variety of miss distances but all with the same $\Delta P_{\rm act}$- and $D_{\rm act}$-values (the exponent $-0.6\pm0.04$ diverges from 0.5 by less than $3-\sigma$). However, such a result is totally unexpected as dust devil diameters are known to span a wide range and would be inconsistent with a variety of studies \citep[cf.][]{2016SSRv..203..277L}. Instead, this result probably only indicates that the range of miss distances plays an important role in setting the observed distribution. 
...
\citet{} and \citet{} develop approaches to de-bias the observed population of vortices and work out the underlying distribution of $\Delta P_{\rm act}$- and $D_{\rm act}$-values. However, they both rely on several plausible but untested assumptions about encounter statistics. Fortunately, the available APSS data themselves provide a more direct way of assessing encounter statistics and inferring the intrinsic vortex properties: high-frequency TWINS wind speed data are available for some (but not all) vortex encounters. As discussed in \citet{2018Icar..299..166J} and \citet{2020AeoRe..4400594F}, the tangential wind speed profile $V(r)$ depends on radial distance $r$ in a different way from the pressure profile (assuming a Rankine vortex profile -- \citealp{1991ExFl...11...73V}):

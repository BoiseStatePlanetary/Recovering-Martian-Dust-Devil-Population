...
\shorttitle{Optimizing In-Situ Dust Devil Surveys}
...
\title{Optimizing In-Situ Dust Devil Surveys}
...
The atmospheric influence of a dust devil population depends on the frequency of dust devil occurrence and the dust flux for each. Thus, accurately assessing that influence requires accurate estimates of the both. We can relate the areal occurrence rate $n$ (dust devils per unit area per unit time) from either the areal density of devils $N$ (measured in dust devils per unit area) or the total number of dust devils observed $k$ in an amount of time $t$:
\begin{equation}
...
A more practical application of Equation \ref{eqn:sigma_n} might instead involve assuming values for $\Delta_{\rm FOV}$ and $\alpha$ to estimate the duration required for a survey of a given precision. Taking the same parameters as above but this time with $\alpha \sim 1\,{\rm rad}$, we require a survey of duration $t > 25\,{\rm hr}$ to achieve $T = 0.1$. In other words, to achieve $T < 0.1$, we would require 
...
\begin{equation}
    \langle Q \rangle = \int_{D_{\rm min}}^{D_{\rm max}} \left( \frac{dk}{dD} \right) q\ \pi D^2 dD = \int_{D_{\rm min}}^{D_{\rm max}} n_0 D^{-\gamma} \left( \frac{t \Delta_{\rm FOV} D^2}{2 \alpha^2} \right) q \pi D^2\ dD.

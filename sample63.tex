...
\added{By design, our detection scheme will filter out some vortex signals. In particular, vortices with pressure signals very different from a Lorentzian will be missed. As an example, a Vatistas vortex that passes over the sensor in a non-linear trajectory would not generate a Lorentzian; however, such encounters seem to be unusual \citep{LORENZ20151}, so we do not consider them. So what about simple Lorentzians -- how many of those has our approach likely to have missed? A simple way to address this question is to consider how many more vortices we might have detected if we reduced our detection threshold $F \ast P$ from 5 to 3. Gaussian statistics for the detections (which is equivalent to assuming white noise) gives a very small fractional increase in probability  }

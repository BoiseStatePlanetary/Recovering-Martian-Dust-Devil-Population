...
\section{Time-Series Analysis and Data}
...
\subsection{Pressure and Wind Models}
\label{sec:Pressure and Wind Models}

As in previous work \citep[e.g.][]{2016JGRE..121.1514K}, we model the pressure signals using a steady-state modified Lorentzian profile:

\begin{equation}
    \Delta P(r) = -\frac{\Delta P_{\rm act}}{1 + \left( \frac{r}{D_{\rm act}/2} \right)^2}\label{eqn:radial_lorentzian_profile}
\end{equation}
where $r$ is the radial distance from the vortex center, $P_{\rm act}$ the central pressure excursion, and $D_{\rm act}$ is taken as the vortex diameter. The closest approach distance between the vortex center and the APSS sensors $b$ is usually greater than zero. As a result, the minimum observed in the pressure time-series $\Delta P_{\rm obs}$ is not usually as deep as the actual pressure excursion at the vortex center \citep{2018Icar..299..166J, 2019Icar..317..209K}. The pressure time-series for a vortex encounter can be represented by 
\begin{equation}
    \Delta P(t) = \frac{-\Delta P_{\rm obs}}{1 + \left( \frac{ t - t_0 }{\Gamma_{\rm obs}/2} \right)^2},\label{eqn:Lorentzian_profile}
\end{equation}
where $t$ is the time of each datum, $t_0$ the time of closest approach ($r(t_0) = b$), and $\Gamma_{\rm obs}$ the observed profile full-width/half-max (FWHM). This profile assumes the vortex travels past the sensor on a linear trajectory with a unidirectional and constant velocity $U$ (at least during the course of a single encounter). Consequently, the radial distance between the vortex center and the barometer $r(t)$ is assumed to evolve as 
\begin{equation}
    r(t) = \sqrt{b^2 + U^2 \left( t - t_0 \right)^2}.\label{eqn:radial_distance}
\end{equation}
With these definitions, $\Gamma_{\rm obs} = U^{-1} \sqrt{D_{\rm act}^2 + \left( 2 b \right)^2}$.

We also model the wind speed time-series using a steady-state vortex. The wind speed signal consists both of an ambient wind $U$ and the vortex wind $V(r)$, which is a function of radial distance (and therefore time). For the Rankine vortex model \citep{1991ExFl...11...73V}, $V(r)$ is given by
\begin{equation}
    V(r) = V_{\rm act} \frac{2 \left( \frac{r}{D_{\rm act}/2} \right) }{1 + \left( \frac{r}{D_{\rm act}/2} \right)^2},
\end{equation}
where $V_{\rm act}$ is the tangential wind speed at the vortex diameter. Similar to the pressure signal, a non-zero $b$ means the maximum wind speed encountered $V_{\rm obs}$ is less than the actual maximum at the vortex diameter. Thus, the observed vortex wind profile is given by 
\begin{equation}
    V(r) = V_{\rm obs} \frac{\sqrt{1 + \left( U/b \right)^2 \left( t - t_0 \right)^2}}{1 + \left(\frac{t - t_0}{\Gamma_{\rm obs}/2}\right)^2}.\label{eqn:wind_profile}
\end{equation}
The total wind speed observed $W(t)$ is the vector sum of the ambient wind and vortex wind, given by 
\begin{equation}
    W(t) = \sqrt{V^2 + 2 U V \cos \theta + U^2},\label{eqn:total_wind_speed}
\end{equation}
where $\cos \theta = b/\sqrt{b^2 + U^2\left( t - t_0 \right)^2}$. 

By fitting these models to the pressure and wind time-series, we can directly estimate $\Delta P_{\rm obs}$ and $\Gamma_{\rm obs}$. In addition, we can estimate the ambient wind speed before and after the vortex encounter ($U_1$ and $U_2$), which, as we show below, are often different. 

The observed pressure and excursions ($\Delta P_{\rm obs}$ and $V_{\rm obs}$) are likely to differ from the intrinsic vortex parameters $\Delta P_{\rm act}$ and $V_{\rm act}$ since $b$ is unlikely zero but otherwise unknown \emph{a priori} for any encounter. However, as we show in Appendix \ref{sec:Inferring Encounter Geometries from the Pressure and Velocity Profiles}, we can estimate the intrinsic from the observed parameters, as well as $b$. To check this approach, we applied these models to many synthetic vortex encounters for a range of encounter geometries, vortex parameters, and time-series noise representative of the observed values. We found that for encounter with $b \lesssim 2 \times D_{\rm act}$, we were able to consistently recover the imposed parameters. Encounters farther than that, and the signals were usually lost in the noise.

As discussed below, the wind time-series have limited time resolution and exhibit correlated noise, both of which make robust model fits difficult. To bolster our results, we fit a second, more simple wind profile:
\begin{equation}
    V(r) = \left\{
        \begin{array}{ll}
            U_1, & \left( t - t_0 \right) < -\Gamma_{\rm obs}\\
            V_{\rm obs} + U_1, & -\Gamma_{\rm obs} \leq \left( t - t_0 \right) \geq \Gamma_{\rm obs}\\
            U_2, & \left( t - t_0 \right) < \Gamma_{\rm obs}\\
        \end{array}
    \right.\label{eqn:simple_wind_profile}
\end{equation}
(For both this simpler and the full wind profile, $V_{\rm obs}$ may be negative, reflecting cyclonic or anti-cyclonic vortices.) We compare the best-fit $V_{\rm obs}$-values between the two models for each encounter to corroborate our results.

Vortex encounters may also produce excursions in the wind direction (as the tangential wind field passes) and in ambient temperature (as the warm core passes over the sensor) \citep{2016SSRv..203...39M}. APSS does return wind direction and air temperature data. However, we do not model these time-series since the wind direction data essentially replicate the encounter information provided by the wind speeds, and, as in other in-situ studies of vortices, the temperature data show small or negligible excursions during the encounters. In any case, the temperatures would be expected to simply mirror the pressure excursions \citep{2016Icar..271..326L}.

\subsection{Data}

The ``model'' wind data files have a sampling frequency of $0.1\,{\rm Hz}$ (i.e., measurements every 10 seconds), while the ``modelevent'' files have a frequency of $1\,{\rm Hz}$.
...
The flattening of the cumulative histogram for $\Delta P_{\rm obs}$ (Figure \ref{fig:DeltaPobs_vs_Gammaobs}(c)) near $\Delta P_{\rm obs} = 1.1\,{\rm Pa}$ indicates a decline in the number detected vortices above that value. This decline occurs, at least in part, because of difficulty detecting these more shallow signals against noise  \citep{2018Icar..299..166J}. Given the possible strong dependence of dust-lifting on $\Delta P$, the exact form of the histogram of $\Delta P_{\rm obs}$-values is critical for evaluating the population's atmospheric influence. We fit a power-law to the cumulative histogram with an exponent $\gamma = -2.39\pm0.02$, which suggests the differential histogram has an exponent $\approx -3.39$. This exponent is consistent with the value reported in  \citet{2020arXiv200501134S} for the same dataset and with the value reported in \citet{2018Icar..299..166J} for the vortices detected by the Phoenix Mission. (On a related note, since the best-fit exponent for the $\Delta P_{\rm obs}$-histogram can depend on the chosen binning, we suggest such analyses use cumulative histograms or a data-informed scheme for binning -- \citealp{2016SSRv..203..277L}.)
...
The $\Gamma_{\rm obs}$- and $\Delta P_{\rm obs}$-values exhibit intriguing trends as well -- Figure \ref{fig:Gammaobs_DeltaPobs_vs_TOD_and_sol}. Figure \ref{fig:Gammaobs_DeltaPobs_vs_TOD_and_sol} (c) shows that, binned by the hour, $\Gamma_{\rm obs}$ increases from $2$ to $20\,{\rm s}$ at a rate of $\left( 1\pm0.1 \right)\,{\rm s\ hr^{-1}}$. Figure \ref{fig:Gammaobs_DeltaPobs_vs_TOD_and_sol} (d) shows that $\Delta P_{\rm obs}$ peaks around 12:00 LTST at $\left( 1.3\pm0.05 \right)\,{\rm Pa}$ with minimum values at either end of about $\left( 0.7\pm 0.1 \right)\,{\rm Pa}$ (where the uncertainties come from the error of the mean in the corresponding bin). The values binned by sol (Figure \ref{fig:Gammaobs_DeltaPobs_vs_TOD_and_sol} (a) and (b)) show no obvious trends. 
...
Presumably, these hour-to-hour trends reflect the influence of variable ambient conditions, but these data alone are not sufficient to judge what influence: the increase in $\Gamma_{\rm obs}$ could result either from intrinsically larger vortices late in the day or lower wind speeds $U$ advecting the same size vortices past the sensor. Fortunately, the TWINS wind speed data can shed light on this issue. Figure \ref{fig:vortices_and_windspeed} shows the vortices with the maximum, median, and minimum $\Gamma_{\rm obs}$-values (panels (a), (b), and (c), respectively). We estimate the advection speed by taking the median value $U$ between 5- and 3-$\Gamma_{\rm obs}$ before the encounter time $t_0$. This approach returns a wind speed close enough in time to be a plausible estimate of the advection speed while widely enough separated in time that the vortex probably did not significantly influence the wind. We then estimate the apparent diameter $D_{\rm obs} = U \Gamma_{\rm obs}$ with uncertainties as $\sigma_{D} = \left( U \Gamma_{\rm obs} \right) \sqrt{ \left( \sigma_U/U \right)^2 + \left( \sigma_{\Gamma_{\rm obs}}/\Gamma_{\rm obs} \right)^2 }$, where $\sigma_U$, the uncertainty on $U$, is estimated from the scatter in $U$ and $\sigma_{\Gamma_{\rm obs}}$ comes from the model estimate. 

\begin{figure}
\centering
\includegraphics[width=0.48\textwidth]{figures/vortices_and_windspeed.png}
\caption{Pressure signals (blue dots) and models (blue lines), along with the corresponding horizontal windspeeds (orange dots). The best-fit wind models based on Equation \ref{eqn:wind_profile} are shown as orange curves, and the simplified wind profile models based on Equation \ref{eqn:simple_wind_profile} are shown as orange horizontal lines. \label{fig:vortices_and_windspeed}}
\end{figure}

Figure \ref{fig:U_vs_t0-sol_hist} shows the wind speeds associated with the observed vortices; the maximum, median, and minimum values are $19.1\pm2.3$, $7.6\pm1.0$, and $0.5\pm0.2 \,{\rm m\ s^{-1}}$, respectively. Figure \ref{fig:Dobs_vs_Gammaobs-DeltaPobs_hist} shows the distribution of $D_{\rm obs}$-values against $\Gamma_{\rm obs}$ and $\Delta P_{\rm obs}$, as well as a cumulative histogram. The maximum, minimum, and median values are $1100 \pm 240$, $75 \pm 21$, and $4 \pm 1 \,{\rm m}$, respectively. Not surprisingly, the $D_{\rm obs}$-values correlate positively with $\Gamma_{\rm obs}$ with scatter resulting from the relatively narrow range of wind speeds. 
...
\begin{equation}
    P_{\rm act} = \left( 1 - \frac{\rho V_{\rm obs}^2}{4 P_{\rm obs}} \right)^{-1} P_{\rm obs},\label{eqn:solution_for_Pact}
\end{equation}
and
\begin{equation}
    V_{\rm act} = \left( 1 - \frac{\rho V_{\rm obs}^2}{4 P_{\rm obs}} \right)^{-1/2} \left( P_{\rm obs}/\rho \right)^{1/2}.\label{eqn:solution_for_Vact}
\end{equation}

We can use these values to infer $b$
\begin{equation}
    b = \left( \frac{D_{\rm act}}{2} \right) \sqrt{\frac{P_{\rm act}}{P_{\rm obs}} - 1},
\end{equation}
...
\begin{equation}
    D_{\rm act} = \frac{D_{\rm obs}}{\sqrt{1 + \left( P_{\rm act}/P_{\rm obs} - 1 \right) }}.\label{eqn:actual_diameter}
\end{equation}

...
\section{How a Vortex Detection and Analysis Scheme Influences the Recovered Population}
Any time-series analysis scheme will unavoidably involve selection effects that can skew the recovered population of signals \citep{2018Icar..299..166J}, so, in addition to outlining our approach here, we explore its biases and how those influence the final recovered population of vortices. As in previous work \citep[e.g.][]{2016JGRE..121.1514K}, we model the vortex signals using a steady-state modified Lorentzian profile $L(t)$:
\begin{equation}
    L(t) = \frac{-\Delta P_{\rm act}}{1 + \left( \frac{ t - t_0 }{\Gamma_{\rm act}/2} \right)^2},\label{eqn:Lorentzian_profile}
\end{equation}
where $t$ is the time of each datum, $\Delta P_{\rm act}$ is the pressure excursion (a positive number) at the center of vortex signal (the ``actual'' value, as contrasted with the ``observed'' value -- see below), $t_0$ the central time, and $\Gamma_{\rm act}$ the actual full-width/half-max (FWHM) for the profile. Assume, also, a simple linear trajectory for the vortex with a unidirectional (but not necessarily constant) wind field of velocity $U$, and take the vortex's physical diameter $D_{\rm act} = U\ \Gamma_{\rm act}$. The radial distance between the barometer and the vortex center $r(t) = \sqrt{U \left( t - t_0 \right)^2 + b^2}$, where $b$ is the closest approach distance during the whole encounter. Figure \ref{fig:} illustrates the encounter geometry.
...
\begin{equation}
    k = 2 N U T b_{\rm max} \approx N U T D_{\rm act} \sqrt{ \frac{\Delta P_{\rm act}}{\Delta P_{\rm min}}}, \label{eqn:number_of_encounters}
\end{equation}
where $T$ is the total duration of the survey (which may consist of multiple observational sequences spread over many sols). If $N$ and $U$ evolve with time (as they usually do), this estimate can be converted into an integral over time. 

As discussed in Section \ref{sec:Introduction}, the quantity of meteorological interest is $N$, which can be estimated from this equation. In particular, we can test predictions for how $N$ should vary with ambient conditions (sensible heat flux, ambient wind speed and shear, boundary layer depth, etc. -- \citealp{2019JGRE..124.3442N}) but only to the extent that we can reliably distinguish different values for $N$. This requirement necessitates an estimate for the uncertainty associated with any estimate of $N$. A reasonable assumption is that vortex encounters represent a Poisson process \citep[cf.][]{Press2007} with an occurrence rate $\nu = N U D_{\rm act} \sqrt{ \Delta P_{\rm act}/\Delta P_{\rm min} } = k/T$. We can estimate the uncertainty on $k$ as $\sigma_k = \sqrt{k}$ and therefrom the uncertainty $\sigma_N$ via
\begin{equation}
    \frac{\sigma_N}{N} = \frac{\sigma_k}{k} = k^{-1/2} = \left ( N U T D_{\rm act} \right)^{-1/2} \left( \Delta P_{\rm act}/\Delta P_{\rm min} \right)^{-1/4}.
    \label{eqn:sigma_N}
\end{equation}

We employ a matched filter approach to recover vortices from the Insight APSS calibrated pressure time-series \citep[][ch.~13]{Press2007}. In addition to Gaussian (white) noise, the data also exhibit long-term variations of timescales typically much longer (minutes to hours) than the vortex signals. The suppress these longer-term signals and facilitate detection of the vortices, we apply a mean boxcar filter with a window size $W$ before sifting the data for vortices. Of course, any time-series analysis will unavoidably involve selection effects that can skew the recovered population of signals \citep{2018Icar..299..166J}, so, in addition to outlining our approach here, we explore its biases and how those influence the final recover population of vortices.

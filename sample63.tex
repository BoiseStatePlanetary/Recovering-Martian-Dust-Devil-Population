...
The ``_model_'' wind data files have a sampling frequency of $0.1\,{\rm Hz}$ (i.e., measurements every 10 seconds), while the ``_modelevent_'' files have a frequency of $1\,{\rm Hz}$.
...
\citet{2018Icar..299..166J}, \citet{2019Icar..317..209K}, and \citet{2020Icar..33513389K} developed approaches to de-bias the observed population of vortices and work out the underlying distribution of $\Delta P_{\rm act}$- and $D_{\rm act}$-values. However, they rely on several plausible but untested assumptions about encounter statistics. Fortunately, the available APSS data themselves provide a more direct way of assessing encounter statistics and inferring the intrinsic vortex properties: high-frequency TWINS wind speed data are available for some (but not all) vortex encounters. As discussed in \citet{2018Icar..299..166J} and \citet{2020AeoRe..4400594F}, the tangential wind speed profile $V(r)$ depends on radial distance $r$ in a different way from the pressure profile (assuming a Rankine vortex profile -- \citealp{1991ExFl...11...73V}):

\begin{equation}
V(r) = V_{\rm T} \frac{2 r / D_{\rm act}}{1 + \left( 2 r / D_{\rm act} \right)^2},\label{eqn:tangential_velocity_profile}
\end{equation}
where $V_{\rm T}$ is the velocity at a radial distance $r = D_{\rm act}$.
...
We assume the vortices correspond to Rankine vortices \citep{1991ExFl...11...73V}, giving pressure $\Delta P$ and tangential wind velocity profiles $V_{\rm r}$:
\begin{equation}
    \Delta P(r) = -\frac{\Delta P_{\rm act}}{1 + \left( \frac{r}{D_{\rm act}/2} \right)^2}\label{eqn:radial_lorentzian_profile}
\end{equation}
and
\begin{equation}
    V_{\rm r}(r) = \frac{ V_{\rm act} \left( \frac{2 r}{D_{\rm act}/2} \right)}{1 + \left( \frac{r}{D_{\rm act}/2} \right)^2},\label{eqn:radial_wind_profile}
\end{equation}
...
\begin{equation}
    V_{\rm r} = \frac{ V_{\rm act} \left( \frac{2 b}{D_{\rm act}/2} \right)}{1 + \left( \frac{b}{D_{\rm act}/2} \right)^2} = V_{\rm obs}.\label{eqn:Vobs}
\end{equation} 

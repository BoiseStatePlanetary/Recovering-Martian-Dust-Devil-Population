...
\added{If the vortex occurrence rate at InSight is similar to the rates seen by other Mars landers, why did InSight image no active dust devils when those other landers imaged many? Not for lack of trying -- \citet{2020NatGe..13..190B} describe concerted imaging campaigns to detect dust devils, as illustrated in Figure \ref{fig:Example-Image_Insight-Combined-Analysis}(b). The ICC sits relatively low to the ground ($0.7\, {\rm m}$), which necessarily limits its horizon. By comparison, the Surface Stereo Imager on the Phoenix lander }

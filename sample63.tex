...
Figure \ref{fig:U_vs_t0-sol_hist} shows the wind speeds associated with the observed vortices, and Figure \ref{fig:Dobs_vs_Gammaobs-DeltaPobs_hist} shows the corresponding distribution of $D_{\rm obs}$-values against  $\Gamma_{\rm obs}$ and $\Delta P_{\rm obs}$, as well as a cumulative histogram. The maximum, minimum, and median values are 1260, 5.43, and $97.3\,{\rm m\ s^{-1}}$, respectively.

Not surprisingly, the $D_{\rm obs}$-values correlate positively with $\Gamma_{\rm obs}$ with scatter resulting from the relatively narrow range of wind speeds. On the other hand, $D_{\rm obs}$ seems to correlate 

...
Next, we couple our pressure profile and wind profile fits (Equations \ref{eqn:Lorentzian_profile} and \ref{eqn:wind_profile}) to infer the actual central pressure excursions $P_{\rm act}$ and vortex diameters $D_{\rm act}$, as well as the the eyewall wind velocities $V_{\rm act}$. For these results, we only included encounters for which the inferred $b/D_{\rm act} \leq 1$, as discussed in Section \ref{sec:Pressure and Wind Models} -- we found \boverDactltone\ such vortex encounters. The best-fit wind profile models are illustrated for the three representative vortex encounters in Figure \ref{fig:vortices_and_windspeed}, Figure \ref{fig:Dact_vs_Pact} shows the distribution of inferred $P_{\rm act}$- and $D_{\rm act}$-values. The 
